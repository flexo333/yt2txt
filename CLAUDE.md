...
- `GET` → list last 50 summaries — a backwards `Query` on the `byCreatedAt` GSI, with the old `Scan` kept as a transitional fallback (see "Things that will bite you"). Each row's `summary` is cut to 300 chars and flagged `truncated: true`; the full markdown comes from `?video=`.
...
- Lambda + IAM role (DDB `PutItem`/`GetItem`/`UpdateItem`/`DeleteItem`/`Scan`/`Query` on the tables and their indexes — `DeleteItem` exists only for the backfill's canonical-key pass) + **Lambda Function URL** (no API Gateway, to avoid the 29s timeout)
...
- **The `?video=` `Scan` fallback is deletable**: `findByVideoId()` `GetItem`s the canonical URL and only falls back to a filtered `Scan` for rows written before canonicalisation. Run the backfill (`{"__backfill": true}`, repeat with `nextStartKey` until `done: true`); once a completed run reports `nonCanonicalRemaining: 0`, no such row exists and everything below the `GetItem` in `findByVideoId()` can go.
- **The history `Scan` fallback is temporary and deletable**: `recentSummaries()` Queries `byCreatedAt` and falls back to the old `Scan` when the index is missing (infra not applied) or still sparse (rows predating `gsi1pk` are absent from a GSI until stamped). Run the backfill (`{"__backfill": true}` — its index pass is pure DynamoDB, no Gemini/YouTube quota, and idempotent), then compare `Table.ItemCount` with `GlobalSecondaryIndexes[0].ItemCount` via `aws dynamodb describe-table --table-name yt2txt-summaries`; once they match, delete `scanRecent()` and the merge in `recentSummaries()`.
