...
- **`listSummaries()` whitelists fields**: it destructures the DynamoDB item, so a new attribute is silently dropped from `GET /` until it is added there *and* to `summaryPayload()`, which backs all three `POST` response paths (fresh, cache hit, and the ConditionalCheckFailed race loser).
