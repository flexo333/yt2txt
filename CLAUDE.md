...
- `GET ?video=<id>` → one summary (full markdown) by YouTube video id: `GetItem` on the canonical `watch?v=` URL, falling back to a filtered `Scan` for rows stored under another URL form. 404s on no match. This is what keeps `/summary/<videoId>` permalinks working past the newest 50.
