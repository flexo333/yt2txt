...
**Date:** 2026-07-31
**Companion spec:** `2026-07-31-merge-person-video-analysis-design.md` — its section 1 ("Enriched prompt") is superseded by this document's prompt text; its slicing contract is preserved unchanged.
...
Output (plain Markdown, in exactly this order):
1. Title: A simple, clear title that says exactly what the video is about.
   No buzzwords. Markdown heading format: '# Title'.
...
Speakers: Jane Doe, John Smith
   Real names only. Skip anyone who is merely mentioned but never speaks,
   and skip generic labels like "host" or "narrator". If you cannot name
   anyone, write exactly "Speakers: none". This line is metadata, not part
   of the summary.
...
- `# Title` is the first heading — `extractTitle()` unchanged.
- The Bottom Line leads, so the history feed's 300-char preview stays meaningful.
- `Speakers:` stays the strippable final line — `tags.js` parse/strip unchanged.
...
1. `backend/summarise/handler.js` — replace the `SYSTEM_PROMPT` string.
...
No parsing contract changes, so the pure suites (`tags`, `share`, `youtube-url`, `people-pure`, `backfill-pure`) are unaffected; `make test` must still pass. Format quality is judged by eye on the next few real summaries — there is no automated check on prompt output shape.
