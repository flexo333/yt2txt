...
Two pins currently block that upgrade:
...
The second is the real obstacle. Relaxing only our own pin will leave pip unable to resolve.
Upstream `lukerohde/pulumi-static-site` has only ever tagged `v0.1.0`.
...
### Option A — bump `pulumi-static-site`, then the provider *(recommended)*

1. Fork or patch `lukerohde/pulumi-static-site`; widen its pin to `pulumi-aws>=6.0.0,<8.0.0`;
   tag `v0.2.0`.
...
4. Run the v7 state migration: `pulumi up --refresh --run-program`.
...
`AGENTS.md:60` documents a hard-won gotcha:
...
This works only because the function is **Pulumi auto-named** — no `name=` argument on the
`aws.lambda_.Function`, so the physical name is `summarise-<hex>`, which matches the
`summarise-*` ARN pattern. That match is incidental. **If the function is ever given an
explicit name outside those three prefixes, CI will start failing with `AccessDenied` on
`AddPermission`**, and the fix belongs in the my-blog ingress stack, not here.
