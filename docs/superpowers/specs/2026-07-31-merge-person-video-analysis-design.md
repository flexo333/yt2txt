...
> 6. What each speaker argues — only when two or more people speak: `## What each speaker argues`, then a `### <Name>` subsection per named speaker with 2–4 bullets of their distinctive viewpoints (≤25 words each) and up to 2 short verbatim-ish quotes with timestamp links. Use exactly the same names here as in the Speakers line. If only one person speaks, omit this section entirely.
...
Cost: video ingestion unchanged; output grows a few hundred tokens on multi-speaker videos only.
